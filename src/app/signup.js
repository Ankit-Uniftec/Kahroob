...
const SignUp = () => {
  ...
  const initialTranslateObj = {
    ...
    HaveAccoutText: `Already member`,
    login: 'Login in',
    signup: 'Sign up',
    errorText: 'Error',
    successText: 'Success',
    strongPassError: 'Strong password required',
  ...
  const handleSignUp = async () => {
    ...
    if (!checkPassword) {
      ...
      showMessage({
        message: translateObj.errorText,
        description: translateObj.strongPassError,
        type: 'danger',
        duration: 3000,
      });
      return;
    ...
    try {
      ...
      console.log('sign up process in>>>>');

      alert('Attempting to connect to the API...');
      ...
      alert('API connection successful. Checking response...');

      if (response.data && response.data.isSuccess) {
        showMessage({
          message: translateObj.successText,
          description: translateObj.accountCreatedText,
          type: 'success',
          duration: 3000,
        });
        setTimeout(() => {
        ...
        alert('API returned an error: ' + response.data.error);
        handleAuthErrors({ code: response.data.error }, translateObj);
      }
    } catch (error) {
      console.log('signup failed ==>', error);
      alert('API connection failed: ' + (error.response ? error.response.data : error.message));
  ...
  const handleGoogleSignUp = async () => {
    // alert('Google signup is not implemented yet Please use email signup');
    try {
      setIsLoading(true);
      ...
      if (verifyResponse.data && verifyResponse.data.isSuccess && verifyResponse.data.data) {
        ...
        console.log('signup failed ==>', verifyResponse.data.error);
        handleAuthErrors({ code: verifyResponse.data.error }, translateObj);
      }
    } catch (apiError) {
      console.log('signup failed ==>', apiError);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {isTranslatesLoaded ? (
        ...
        >
          <Text style={styles.title}>{translateObj.createAccount}</Text>
          <Text style={styles.subtitle}>{translateObj.signupText}</Text>
          ...
          <View style={styles.inputContainer}>
            ...
            <InputField
              placeholder={translateObj.password}
              iconname={'lock'}
              value={password}
          ...
          <CustomButton title={translateObj.signup} onPress={handleSignUp} loading={isLoading} />

          <Text style={styles.or}>{translateObj.continueWith}</Text>
          <View style={styles.pngHolderMain}>
            <TouchableOpacity onPress={() => handleGoogleSignUp()} style={styles.pngButton}>
          ...
          >
            ...
            >
              <Text>{translateObj.HaveAccoutText}? </Text>
            </View>
            ...
            >
              <Text style={styles.signup}>{translateObj.login}</Text>
            </TouchableOpacity>
          </View>
        </ScrollView>
      ) : (
