import { StyleSheet, Text, View, useWindowDimensions, Image, ScrollView, FlatList, Share } from 'react-native'
...
import { BOOKMARK_ARTICLES } from '../store/allactionsTypes';
...
const TipsandtricksDetails = () => {
    ...
    const { articles } = useSelector(state => state.ThemeReducer)

    let findIndex = articles.findIndex(f => f.userId === user?.uid);
    const [isFavorite, setIsFavorite] = useState(findIndex !== -1);

    const handleShare = async () => {
        try {
            const result = await Share.share({
                message: `Check out this classified vehical: BMW • X1`,
            });
    ...
    const handleFavorite = async () => {
        if (findIndex !== -1) {
            ...
            updatedArticles.splice(findIndex, 1);
            dispatch({ type: BOOKMARK_ARTICLES, payload: updatedArticles });
            setIsFavorite(false);
        } else {
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: ThemeConstant.PADDING_MAIN, paddingBottom: moderateVerticalScale(20) }}>
                <View style={{ flexDirection: 'row', justifyContent: "space-between", alignItems: 'center' }}>
                    ...
                    <View style={{ flexDirection: 'row', gap: scale(30) }}>
                        ...
                        <FontAwesome onPress={handleFavorite} name={isFavorite ? "bookmark" : "bookmark-o"} size={24} color={!isFavorite ? '#000' : '#00CC44'} />
                    </View>
                </View>
