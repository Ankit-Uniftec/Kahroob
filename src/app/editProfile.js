...
import { UPDATE_USER_DETAILS } from '../store/allactionsTypes';
...
const EditProfile = () => {
  ...
  const initialTranslateObj = {
    ...
    userSignInText: 'User not signed in',
    successText: 'Success',
    profileUpdateText: 'Your profile has been updated!',
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (!user) {
        throw new Error(translateObj.userSignInText);
      }
      ...
      dispatch({ type: UPDATE_USER_DETAILS, payload: { ...user, name: fullName, phone, countryCode } });
      setIsLoading(false);
      showMessage({
        message: translateObj.successText,
        description: translateObj.profileUpdateText,
        type: 'success',
        duration: 3000,
      });
      router.back();
    ...
    <SafeAreaView edges={['top']} style={styles.container}>
      {isTranslatesLoaded ? (
        <>
          ...
          >
            <BackButton />
            <Text style={{ fontWeight: '600', fontSize: scale(20) }}>{translateObj.personalInfoText}</Text>
          </View>
          ...
          <ScrollView
            contentContainerStyle={{ flexGrow: 1, paddingBottom: 30, paddingHorizontal: ThemeConstant.PADDING_MAIN }}
          >
            <View style={styles.inputContainer}>
              {/* Full Name */}
              <View>
                <Text style={styles.label}>{translateObj.fullNameText}</Text>
                <InputField
                  placeholder={translateObj.fullNameText}
                  value={fullName}
              ...
              {/* Email */}
              <View>
                <Text style={styles.label}>{translateObj.emailText}</Text>
                <InputField
                  placeholder={translateObj.emailText}
                  value={user?.email}
              ...
              {/* Phone No. */}
              <View>
                <Text style={styles.label}>{translateObj.phoneText}</Text>
                <View
                  style={{
                    flexDirection: 'row',
                    alignItems: 'center',
                ...
                >
                  ...
                  <View style={{ width: '75%', paddingLeft: scale(3) }}>
                    <InputField
                      placeholder={translateObj.phoneText}
                      value={phone}
            ...
            <CustomButton title={translateObj.saveChangesText} onPress={handleSubmit} loading={isLoading} />
          </ScrollView>
        </>
      ) : (
